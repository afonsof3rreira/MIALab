...
def to_sitk_image(np_image, reference_img):
    ...
    img.CopyInformation(
        reference_img)  # todo:. this is important bc image is not only pixels, we need origin, direction, etc
...
def register_images(img, label_img, atlas_img):
    ...
    registered_img = sitk.Resample(img, atlas_img, transform, sitk.sitkLinear, 0.0,
                                   img.GetPixelIDValue())  # todo: modify here
    ...
    registered_label = sitk.Resample(label_img, atlas_img, transform, sitk.sitkNearestNeighbor, 0.0,
                                     label_img.GetPixelIDValue())  # todo: modify here
...
def preprocess_rescale_numpy(np_img, new_min_val, new_max_val):
    ...
    rescaled_np_img = new_min_val + abs(new_max_val - new_min_val) * (np_img - min_val) / (
                max_val - min_val)  # todo: modify here
